...
void calc_projs2(unsigned long int nbr_grid_pts, unsigned long int N, double grid_spacing, double *x, double *p, matrix *rho_thermal, matrix **Pi, matrix *proj_sum) {
	...
	for (i=0; i<nbr_grid_pts; i++) {
		for (j=0; j< nbr_grid_pts; j++) {
			// allocate 0 matrix
			Pi[i*nbr_grid_pts + j] = make_matrix(N);
			//	printf("hej\n");
		    set_zero(D);
			displacement(x[i], p[j], N,D);
		   	//printf("xi %f pj %f i %i j %i \n",x[i],p[j],i, j);
			...
			*/
			...
			cblas_zgemm3m(CblasRowMajor, CblasNoTrans, CblasNoTrans, N, N, N, &const1, D->data, N, tmp1->data, N, &const2, Pi[i*nbr_grid_pts + j]->data, N);
			...
			*/
			// divide by pi, and multipy with grid_spacing^ (because of integral approx)
			cblas_zdscal(N*N, grid_spacing*grid_spacing/M_PI, Pi[i*nbr_grid_pts + j]->data, 1);
			...
			*/
			...
			// sum all projectors
			vzAdd(N*N, proj_sum->data, Pi[i*nbr_grid_pts + j]->data, proj_sum->data);
...
void calc_projs_diag(unsigned long int nbr_grid_pts, unsigned long int N, double grid_spacing, double *x, double *p, matrix *rho_thermal, MKL_Complex16 **Pi_diag, MKL_Complex16 *Pi_sum) {
	...
	matrix *D_dagger = make_matrix(N);
	matrix *tmp1 =  make_matrix(N); // for intermediate step in multipication
	MKL_Complex16 *Pi_tmp = mkl_malloc(N*N*sizeof(MKL_Complex16),64);
	MKL_Complex16 scale; scale.real=1; scale.imag=0; // for hermitian conjugate, no scaling
	double const1 = 1;
	double const2 = 0;
	...
	for (i=0; i<nbr_grid_pts; i++) {
		...
		printf("D\n");
			...
			// reset Pi_tmp
			memset(Pi_tmp, 0,N*N*sizeof(MKL_Complex16));

			cblas_zgemm3m(CblasRowMajor, CblasNoTrans, CblasNoTrans, N, N, N, &const1, D->data, N, tmp1->data, N, &const2, Pi_tmp, N);



			printf("Pi_tmp\n");
			for (k=0;k<N;k++){
				printf("%f %f %f \n",Pi_tmp[k*N+0].real,Pi_tmp[k*N+1].real,Pi_tmp[k*N+2].real);
			}


			// copy diagonal of diagonal matrix Pi_tmp into array
			printf("xi %f pj %f i %i j %i \n",x[i],p[j],i, j);
			for (k=0; k<N; k++) {
				Pi_diag[i*nbr_grid_pts + j][k]=Pi_tmp[k*N + k];
				printf("%f \n",Pi_tmp[k*N + k].real);
			}
			...
			// divide by pi, and multipy with grid_spacing^ (because of integral approx)
			cblas_zdscal(N*N, grid_spacing*grid_spacing/M_PI, Pi_tmp, 1);

			//printf("he4j\n");

			// sum all projectors
			vzAdd(N*N, Pi_sum, Pi_diag[i*nbr_grid_pts + j], Pi_sum);
	...
	for (k=0; k<iterations; k++) {
		...
		for (i=0; i<nbr_grid_pts; i++) {
			for (j=0; j<nbr_grid_pts; j++) {
				...
				set_zero(pr_matrix);
				cblas_zgemm3m(CblasRowMajor, CblasNoTrans, CblasNoTrans, Nfock, Nfock, Nfock, &alpha, rho->data, Nfock, Pi[i*nbr_grid_pts + j]->data, Nfock, &beta, pr_matrix->data, Nfock);
				...
				if (pr > cutoff ) {
					...
					cblas_zdscal(Nfock*Nfock,  histogram[i*nbr_grid_pts + j] / pr, Pi[i*nbr_grid_pts + j]->data, 1);
